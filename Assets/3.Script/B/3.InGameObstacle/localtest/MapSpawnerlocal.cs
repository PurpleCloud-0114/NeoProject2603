...
public class MapSpawnerlocal : MonoBehaviour
{
    [Header("연동 스크립트")]
    [SerializeField] private ObstacleSpawnerlocal _obstacleSpawner;
    ...
    public List<MapFloorlocal> SpawnedFloors { get; private set; } = new List<MapFloorlocal>();
    ...
    public void FullGenerate()
    {
        ...
        while (currentY > _endY)
        {
            ...
            MapFloorlocal mapFloor = floorObj.GetComponent<MapFloorlocal>();
            if (mapFloor != null)
            {
        ...
        Debug.Log("[Local] 전체 맵 생성 완료 (스케일 변화 적용됨)");
    }
